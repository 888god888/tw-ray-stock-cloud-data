...
async function dbSave(v){const d=await dbOpen();return new Promise((ok,no)=>{const t=d.transaction('data','readwrite');t.objectStore('data').put(v,'latest');t.oncomplete=()=>{d.close();ok()};t.onerror=()=>no(t.error)})}
async function dbLoad(){const d=await dbOpen();return new Promise((ok,no)=>{const r=d.transaction('data').objectStore('data').get('latest');r.onsuccess=()=>{d.close();ok(r.result||null)};r.onerror=()=>no(r.error)})}
...
function setup(){
 ...
 const previous=$('industry').value;const inds=['全部'].concat(Array.from(new Set(snapshot.stocks.map(s=>s.industry))).sort((a,b)=>a.localeCompare(b,'zh-Hant')));$('industry').innerHTML=inds.map(x=>`<option>${esc(x)}</option>`).join('');if(inds.includes(previous))$('industry').value=previous;
...
function writeSavedStrategies(items){localStorage.setItem(SAVED_STRATEGIES_KEY,JSON.stringify(items))}
...
function evaluateCondition(s,c){
 ...
 if(c.id.startsWith('eps_')){
  const values=eps.map(x=>num(x.value));const n=Math.max(1,num(p.n));if(c.id==='eps_consec_yoy'&&values.length<n+4)return fail('EPS資料不足');if(c.id!=='eps_consec_yoy'&&c.id==='eps_consec_qoq'&&values.length<n+1)return fail('EPS資料不足');if(values.length<n)return fail('EPS資料不足');
...
function render(){if(!snapshot)return;const q=$('search').value.trim().toLowerCase(),ind=$('industry').value,sort=$('sort').value;let rows=(screenedStocks===null?snapshot.stocks:screenedStocks).filter(s=>(!q||`${s.stock_id} ${s.name}`.toLowerCase().includes(q))&&(ind==='全部'||s.industry===ind));const cmp={change_desc:(a,b)=>b.change_pct-a.change_pct,change_asc:(a,b)=>a.change_pct-b.change_pct,capital_desc:(a,b)=>num(b.capital_billion,-Infinity)-num(a.capital_billion,-Infinity),volume_desc:(a,b)=>b.volume_lots-a.volume_lots,price_desc:(a,b)=>b.close-a.close,code_asc:(a,b)=>a.stock_id.localeCompare(b.stock_id)};rows.sort(cmp[sort]);$('visibleCount').textContent=`符合 ${rows.length} 檔`;$('stockList').innerHTML=rows.map(s=>`<button class="stock-card" data-id="${esc(s.stock_id)}"><div class="identity"><span class="code">${esc(s.stock_id)}</span><b>${esc(s.name)}</b><small>${esc(s.industry)}</small></div><div class="quote"><b>${s.close.toFixed(2)}</b><span class="change ${s.change_pct>=0?'up':'down'}">${pct(s.change_pct)}</span></div><div class="card-metrics"><span>股本<b>${fmt(s.capital_billion,2)} 億</b></span><span>成交量<b>${fmt(s.volume_lots,1)} 張</b></span><span>符合<b>${esc(s.match||'—')}</b></span></div></button>`).join('')||'<div class="empty-state">目前沒有符合股票</div>';$('stockList').querySelectorAll('[data-id]').forEach(b=>b.onclick=()=>openDetail(rows.find(s=>s.stock_id===b.dataset.id)))}
function openDetail(s){selected=s;$('detail').hidden=false;$('detailCode').textContent=s.stock_id;$('detailName').textContent=s.name;$('detailIndustry').textContent=s.industry;$('detailClose').textContent=s.close.toFixed(2);$('detailChange').textContent=pct(s.change_pct);$('detailChange').className=s.change_pct>=0?'up':'down';$('detailCapital').textContent=`${fmt(s.capital_billion,2)} 億`;$('detailVolume').textContent=`${fmt(s.volume_lots,1)} 張`;$('detailMatch').textContent=s.match||'—';$('eps').innerHTML=(s.eps||[]).slice(-8).reverse().map(x=>`<div><span>${esc(x.date)}</span><b>${num(x.value).toFixed(2)}</b></div>`).join('')||'沒有 EPS 資料';$('detailConditions').innerHTML=((s._screenDetails&&s._screenDetails.length?s._screenDetails:s.details)||[]).map(x=>`<li>${esc(x)}</li>`).join('')||'<li>沒有條件明細</li>';renderChip(s.institutional||[]);requestAnimationFrame(()=>{drawCandle();drawRevenue()})}
...
function drawRevenue(){if(!selected)return;const d=(selected.monthly_revenue||[]).slice(-12);const {x,w,h}=canvas('revenue');x.clearRect(0,0,w,h);if(!d.length){x.fillStyle='#91a7b5';x.fillText('沒有月營收資料',20,30);return}const max=Math.max(10,...d.reduce((a,p)=>a.concat([Math.abs(num(p.mom_pct)),Math.abs(num(p.yoy_pct))]),[])),mid=h*.5,scale=h*.32/max,g=(w-32)/d.length;x.strokeStyle='#5e7781';x.beginPath();x.moveTo(16,mid);x.lineTo(w-16,mid);x.stroke();d.forEach((p,i)=>{const X=18+i*g,m=num(p.mom_pct),y=num(p.yoy_pct);x.fillStyle='#26c5aa';x.fillRect(X,m>=0?mid-m*scale:mid,g*.3,Math.max(Math.abs(m*scale),1));x.fillStyle='#efb65a';x.fillRect(X+g*.35,y>=0?mid-y*scale:mid,g*.3,Math.max(Math.abs(y*scale),1));x.fillStyle='#829aa7';x.font='9px sans-serif';x.fillText(String(p.date).slice(5,7),X,h-12)})}
...
['search','industry','sort'].forEach(id=>$(id).addEventListener(id==='search'?'input':'change',render));
$('closeDetail').onclick=()=>{$('detail').hidden=true;selected=null};$('days').onchange=drawCandle;
